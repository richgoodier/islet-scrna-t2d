...
> **Note:** ENA downloads may produce filenames with `%0D` URL encoding appended. Strip it before proceeding:
>
> ```bash
> for file in *; do mv "$file" "$(echo "$file" | sed 's/%0D//')"; done 2>/dev/null
> ```
...
This outputs `read_counts_filtered.csv`. Samples with fewer than 750,000 reads were excluded.

## Notebooks

- `create_download_scripts.ipynb` — how the download scripts were generated from the full ENA manifest
